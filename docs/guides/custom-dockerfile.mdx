...
If the base image lives in a private registry, set [`EXTERNAL_REGISTRY_URL` / `EXTERNAL_REGISTRY_USERNAME` / `EXTERNAL_REGISTRY_PASSWORD`](../configuration/environment-variables#external-private-registry) on the build so the builder can authenticate before pulling.
