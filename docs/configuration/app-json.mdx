...
| `addons` | Service dependencies (databases, caches, etc.) |
...
The `addons` field declares service dependencies for your application. Each addon can be specified as a simple string or as an object with additional configuration.
...
| Property | Description |
|----------|-------------|
| `plan` | The addon plan identifier (e.g., `heroku-postgresql:hobby-dev`) |
| `as` | Attachment alias — overrides the default config var prefix |
| `options` | Provider-specific configuration options |

<Note>
  String-format addons are normalized to object format in the result JSON with `as` and `options` set to `null`.
</Note>
...
```json
{
  ...
  "addons": [
    {"plan": "heroku-postgresql:hobby-dev", "as": null, "options": null},
    {"plan": "heroku-redis:premium-0", "as": "CACHE", "options": null}
  ],
  "language": "ruby",
